...
//     <h4>{`${summarizerState.downloaded === 100 ? "Loaded summarizer model!" : `Downloading summarizer model:${summarizerState.downloaded}`}%`}</h4>
//     </div>
//     </>
//   )
...
const App = ()  =>{
  ...
  const downloadLangDetectModel = async() => {
    ...
    if (canDetect === "no") {
      ...
      const detector = await self.ai.languageDetector.create({
        monitor(m) {
          m.addEventListener("downloadprogress", (e) => {
            setDetectorState({modelState:"downloading", downloaded: ((e.loaded / e.total) * 100)});
          });
        },
      });
      await detector.ready;
  ...
  // UseEffect to check and load models
  useEffect(() => {
    if ('ai' in self && 'languageDetector' in self.ai) {
